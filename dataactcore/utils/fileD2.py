...
def temp_query_data(session, agency_code, start, end, page_start, page_stop):
    """ Request D2 file data
    ...
    rows = session.query(
        ...
        func.to_char(cast(file_model.modified_at, Date), 'YYYYMMDD')).\
        filter(file_model.is_active.is_(True)).\
        filter(file_model.awarding_agency_code == '020').\
        filter(cast(file_model.action_date, Date) >= cast('20171001', Date)).\
        filter(cast(file_model.action_date, Date) <= cast('20171031', Date)).\
        slice(0, 5)
    return rows
